document.addEventListener("DOMContentLoaded", () => {
  ...
  function buildOrderHTML() {
    ...
    if (customer.giftSize) {
      html += `
        <hr/>
        <p><strong>🎁 Подарок (Nike Mind 001):</strong> размер ${escapeHtml(customer.giftSize)}</p>
      `;
    }

    html += `<p><em>Статус: ожидает проверки оплаты</em></p>`;
    return html;
  }
  ...
  async function sendOrderViaEmail() {
    const templateParams = {
      to_email: "vs29062009@gmail.com",   // ← СЮДА ВАШ EMAIL
      customer_name: customer.name || "—",
      customer_phone: customer.phone || "—",
      customer_address: customer.address || "—",
      order_html: buildOrderHTML(),
      order_total: total.toLocaleString() + " ₽",
      gift_size: customer.giftSize || "не выбран" // добавляем отдельный параметр для шаблона
    };
